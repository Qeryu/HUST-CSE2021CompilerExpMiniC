...
Specifier: TYPE {
        ...
        | StructSpecifier {
            $$ = new NSpecifier($1->parse()); 
            $$->line = $1->line;
        }   
        ;

StructSpecifier: STRUCT OptTag LC DefList RC {
            $$ = new NStructSpecifier($2, $4);
            $$->line = $2->line;
        }   
        | STRUCT Tag {
            $$ = new NStructSpecifier($2);
            $$->line = $2->line;
        }
        ;
...
OptTag: ID { 
            std::string *strID = new std::string($1); 
            $$ = new NIdentifier(*strID); 
            $$->line = yylineno-1; 
        }   
        | {
            $$ = nullptr;
...
FunDec: ID LP VarList RP {
            std::string *strID = new std::string($1); 
            NIdentifier *nIdentifier = new NIdentifier(*strID);
            $$ = new NFunDec(*nIdentifier, $3);
            $$->line = $3->line;
        }
        | ID LP RP {
            std::string *strID = new std::string($1); 
            NIdentifier *nIdentifier = new NIdentifier(*strID);
            $$ = new NFunDec(*nIdentifier, nullptr);
            $$->line = yylineno;
        }
        ;
VarList: ParamDec {
            $$ = new NVarList(*$1, nullptr);
            $$->line = $1->line;
        }
...
Exp:      Exp ASSIGNOP Exp {
            ...
            $$ = new NAssignment(name, *$1, ASSIGNOP, *$3);
            $$->line = $1->line;
        }//$$结点type_id空置未用，正好存放运算符
        
        | Exp PLUSASS Exp   {
            std::string name = "PLUSASS";
            $$ = new NAssignment(name, *$1, PLUSASS, *$3);
            $$->line = $1->line;
        }//复合赋值运算
        | Exp MINUSASS Exp   {
            std::string name = "MINUSASS";
            $$ = new NAssignment(name, *$1, MINUSASS, *$3);
            $$->line = $1->line;
        }
        | Exp STARASS Exp   {
            std::string name = "STARASS";
            $$ = new NAssignment(name, *$1, STARASS, *$3);
            $$->line = $1->line;
        }
        | Exp DIVASS Exp   {
            std::string name = "DIVASS";
            $$ = new NAssignment(name, *$1, DIVASS, *$3);
            $$->line = $1->line;
        }
        ...
        | Exp AND Exp   {
            std::string name = "AND";
            $$ = new NBinaryOperator(name, *$1, AND, *$3);
            $$->line = $1->line;
        }
        | Exp OR Exp    {
            std::string name = "OR";
            $$ = new NBinaryOperator(name, *$1, OR, *$3);
            $$->line = $1->line;
        }
        | Exp RELOP Exp {
            std::string name = "RELOP";
            $$ = new NBinaryOperator(name, *$1, RELOP, *$3);
            $$->line = $1->line;
        }  //词法分析关系运算符号自身值保存在$2中
        | Exp PLUS Exp  {
            std::string name = "PLUS";
            $$ = new NBinaryOperator(name, *$1, PLUS, *$3);
            $$->line = $1->line;
        }
        | Exp MINUS Exp {
            std::string name = "MINUS";
            $$ = new NBinaryOperator(name, *$1, MINUS, *$3);
            $$->line = $1->line;}
        | Exp STAR Exp  {
            std::string name = "STAR";
            $$ = new NBinaryOperator(name, *$1, STAR, *$3);
            $$->line = $1->line;
        }
        | Exp DIV Exp   {
            std::string name = "DIV";
            $$ = new NBinaryOperator(name, *$1, DIV, *$3);
            $$->line = $1->line;
        }
        ...
        | MINUS Exp %prec UMINUS   {
            std::string name = "MINUS";
            $$ = new NSingleOperator(name, MINUS, *$2);
            $$->line = $2->line;
        }//这里利用BISON %prec表示和UMINUS同优先级 相当于虚拟出一个运算符
        | NOT Exp       {
            std::string name = "NOT";
            $$ = new NSingleOperator(name, NOT, *$2);
            $$->line = $2->line;
        }
        | ID LP Args RP {
            auto strID = new std::string($1);
            auto id = new NIdentifier(*strID);
            $$ = new NMethodCall(*id, $3);
            $$->line = $2;
        }
        | ID LP RP      {
            auto strID = new std::string($1);
            auto id = new NIdentifier(*strID);
            $$ = new NMethodCall(*id, nullptr);
            $$->line = $2;
        }
        | Exp LB Exp RB { $$ = new NListOperator(*$1, *$3); $$->line = $1->line; }
        | Exp DOT ID    { 
            std::string *strID = new std::string($3); 
            NIdentifier *nIdentifier = new NIdentifier(*strID);
            $$ = new NDotOperator(*$1, *nIdentifier);
            $$->line = $1->line;
        }
        | ID            {
            auto strID = new std::string($1);
